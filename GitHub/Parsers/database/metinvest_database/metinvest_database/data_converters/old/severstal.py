...
PARSER_CODE = 'severstal'

CATEGORY_FIELD = 'Категория'
SECTION_FIELD = 'Подкатегория'
...
def create_all():
  ...
  for pre_product in pre_products.all():
    ...
    united_category = '{}, {}'.format(category_name, section_name)
    ...
    #price = ','.join([data.get(price, '') for price in PRICE_FIELDS])
    price = ''
