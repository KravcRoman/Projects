...
from metinvest_database.constants import Constant
...
from metinvest_database.logger import log
...
def replace_json_data(words, json_data):
  ...
  for key in json_data.keys():
    if json_data[key]:
      if key == Constant.DATA:
        result[key] = {k: replace_with_selected_by_html_word(words, v, prepare_for_search(v)) for k, v in json_data[key].items()}
      elif key == 'html' or key == Constant.CITY:
        result[key] = replace_with_selected_by_html_word(words, json_data[key], prepare_for_search(json_data[key]))
      elif key == Constant.CATEGORY or key == Constant.SUPPLIER:
        result[key] = json_data[key]
        result[key][Constant.NAME] = replace_with_selected_by_html_word(words, json_data[key][Constant.NAME], prepare_for_search(json_data[key][Constant.NAME]))
...
def search(request):
  if request.POST and 'query' in request.POST:
    log.debug(f"Request body: {request.POST.dict()}")
...
def count(request):
  if request.POST and 'query' in request.POST:
    ...
    log.debug(f"Provided cities: {cities}")
    if query:
      ...
      log.debug(f"Total products: {products_total}")
...
def generate_pdf(request):
    if request.method == 'POST':
        #try:
            ...
            log.debug(f"static_url: {static_url}")
            context = {
