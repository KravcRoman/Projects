...
'''
PARSER_NAME = 'inrost_group'

PARSER_FILENAME = 'inrost_group.py'

PARSER_IP = '92.63.179.116'
...
def get_last_csv_file_name() -> list:
    '''
    ...
    try:
        ...
        csv_files_in_curr_dir = [
            csv_files for csv_files in files_in_curr_dir if csv_files[-4:] == '.csv'
        ]
        return csv_files_in_curr_dir
...
def get_data_from_all_CSVs() -> list:
    '''
    ...
    try:
        data = []
        csv_files_in_curr_dir = get_last_csv_file_name()
        for csv_file in csv_files_in_curr_dir:
        ...
        logging.error(f'in get_data_from_last_csv(): {e}')
        send_error_to_cmc(f'in get_data_from_last_csv(): {str(e)[:50]}')
    ...
    try:
        ...
        for chunk in split_into_chunks():
            ...
            while not success_send:
                chunk_with_parser = {'Parsers1': PARSER_NAME, 'data': chunk}
                ...
                chunk_with_parser_json = json.dumps(chunk_with_parser, ensure_ascii=False)
                response = session.post(ADAPTER_FULL_URL, data=chunk_with_parser_json.encode('utf-8'))

                if response.status_code == 400:
                    try:
                        response_text = response.text
                        if '<title>' in response_text:
                            title_index = response_text.index('<title>')
                            end_title_index = response_text.index('</title>')
                            django_error = ' '.join(response_text[title_index + 7:end_title_index].replace('\n', '').split())
                    except Exception:
                        pass
                ...
                if response_data.get('result') == 'ok':
                    ...
                    if count == 1:
                        ...
                        time.sleep(delay)
                        logging.error(f'in send_data_to_adapter() try: {count}, status: {response.status_code}, {response.text}')
                        send_error_to_cmc(f'in send_data_to_adapter()\ntry: {count}, status: {response.status_code}\n{response.text[:50]}')
                    elif count > 2:
                        time.sleep(delay)

        session.close()
        send_status_of_finished_session()
        print(f'Products was sends, chunks = {chunk_count}, products = {product_count}')
    except Exception as e:
        logging.error(f'in send_data_to_adapter(): {e}, django error: {django_error}')
        send_error_to_cmc(f'in send_data_to_adapter(): {str(e)[:50]}, {django_error}')
