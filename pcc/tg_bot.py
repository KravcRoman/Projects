...
parsers_status_dict = {
    ...
    'sds_center': 'Awaiting Start',
    'intormetall': 'Awaiting Start',
    'inrost_group': 'Awaiting Start',
    'metall_service': 'Awaiting Start',
    'russteels': 'Awaiting Start',
    'mmk': 'Awaiting Start',
    'santech': 'Awaiting Start',
    'nlmk': 'Awaiting Start',
    'medexe': 'Awaiting Start',
    'transsibmetall': 'Awaiting Start',
    'ileko': 'Awaiting Start',
    'listmet': 'Awaiting Start',
    'orinnox': 'Awaiting Start',
    'kontinental': 'Awaiting Start',
    'steelx': 'Awaiting Start',
    'ostm': 'Awaiting Start',
}

# Относительный путь к расписанию запусков
...
logging.basicConfig(filename=CURRENT_DIR + f'/errors_tg_bot.log',
...
def main_menu(message: telebot.types.Message):
    ...
    try:
        ...
        # Текст сообщения
        content = 'Выберите пункт для настройки'

        # Изображение
        photo = open(pictures_folder_path + 'menu.jpg', 'rb')
        
        # Приветственное сообщение с выводом кнопок и изображения
        bot.send_photo(message.chat.id, photo, caption=content,
                       reply_markup=markup)


    # Ловим ошибку
    except Exception as e:
        logging.error(f'in main_menu(): {e}')


@bot.message_handler(commands=['start_parsers'])
def start_parsing(message: telebot.types.Message):
    '''
    Обработчик команды /start_parsers для запуска скрипта через CRON из ЦУП
    ...
    '''
    try:
        # Добавляем задачу в крон, получаем его ответ строкой
        cron_answer = add_cron_job()

        # Отвечаем в чат
        bot.send_message(message.chat.id, cron_answer)
...
def stop_parsing(message: telebot.types.Message):
    '''
...
def current_status_cmc() -> dict:
    ...
    '''
    try:
        # Формируем название файла текущих суток для проверки
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in current_status_cmc(): {e}')


@bot.message_handler(commands=['status'])
def current_status(message: telebot.types.Message):
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in current_status(): {e}')


def info_logs_cmc(day: str) -> str:
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in info_logs_cmc(): {e}')


@bot.message_handler(commands=['info'])
def info_logs_menu(message: telebot.types.Message):
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in info_logs_menu(): {e}')


def info_logs(message: telebot.types.Message):
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in info_logs(): {e}')


@bot.message_handler(commands=['launch_plan'])
def start_list(message: telebot.types.Message):
    '''
    ...
    try:
        ...
        current_moscow_time = datetime.datetime.now(moscow_tz).strftime('%H:%M:%S')
        ...
        try:
            ...
            text_to_send = f'`  Время мск {current_moscow_time}`\n\n'
            count = 1
            for parser_name, parser_start_time in sorted_parsers:
                count_and_parser = f'{count} {parser_name}'
                text_to_send += f'`{count_and_parser.title():<17} ' \
                                f'{parser_start_time}\n`'
                count += 1
    ...
    # Ловим ошибку функции
            ...
            logging.error(f'in start_list(): {e}')


@bot.message_handler(commands=['change_launch_plan_menu'])
def change_launch_plan_menu(message: telebot.types.Message):
    '''
    ...
    try:
        ...
        try:
            ...
            sorted_parsers = sorted(
                parsers_start_plan_dict.items(),
                key=lambda x: datetime.datetime.strptime(x[1], '%H:%M')
            )
    ...
    # Ловим ошибку функции
            ...
            logging.error(f'in change_launch_plan_menu(): {e}')
...
def change_launch_plan(message: telebot.types.Message):
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in change_launch_plan(): {e}')


def start_time_change(message: telebot.types.Message):
    '''
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in start_time_change(): {e}')
...
def handle_message(message: telebot.types.Message):
    ...
    # Ловим ошибку функции
        ...
        logging.error(f'in handle_message(): {e}')
