...
PARSER_CODE = 'intormetall'

CATEGORY_FIELD = 'Основной раздел'
SECTION_FIELD = 'Подраздел'
SUBSECTION_FIELD = 'Подраздел 2'

PRICE_FIELD = 'Цена'

NAME_FIELD = 'Наименование'

EXCLUDE_KEYS = [CATEGORY_FIELD, NAME_FIELD, PRICE_FIELD]
...
def create_all():
  ...
  for pre_product in pre_products.all():
    ...
    city_name = 'Долгопрудный' #data[SUPPLIER_FIELD].split('(')[-metinvest].strip().split(')')[-2]
    ...
    supplier_name = 'Инторметалл' #data[SUPPLIER_FIELD].split('(')[-2].strip()
    ...
    subsection_name = data[SUBSECTION_FIELD]

    united_category = '{}, {}, {}'.format(category_name, section_name, subsection_name)
