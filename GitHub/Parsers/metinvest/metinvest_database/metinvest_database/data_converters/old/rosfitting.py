...
PARSER_CODE = 'rosfitting'

PRICE_FIELD = 'price'

NAME_FIELD = 'name'

EXCLUDE_KEYS = [NAME_FIELD, PRICE_FIELD]
...
def create_all():
  ...
  for pre_product in pre_products.all():
    ...
    city_name = 'Обнинск/Люберцы(Красково)' #data[SUPPLIER_FIELD].split('(')[-metinvest].strip().split(')')[-2]
    ...
    supplier_name = 'Росфиттинг' #data[SUPPLIER_FIELD].split('(')[-2].strip()
    ...
    united_category = "Фиттинг" #'{}, {}, {}'.format(category_name, section_name, subsection_name)
