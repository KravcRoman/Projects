...
EXCLUDE_KEYS = [CATEGORY_FIELD, NAME_FIELD, PRICE_FIELDS[0], PRICE_FIELDS[1]]
...
def create_all():
  ...
  for pre_product in pre_products.all():
    ...
    city_name = 'Москва' #data[SUPPLIER_FIELD].split('(')[-1].strip().split(')')[-2]
    ...
    supplier_name = 'Грандлайн' #data[SUPPLIER_FIELD].split('(')[-2].strip()
